/**
 * BusJob - 「求人」投稿のレイアウトを会社紹介記事(bj-article)形式に一括差し替え(1回だけ実行するスニペット)
 *
 * 対象: post_type = job のうち、タイトルが一致する既存投稿(下書き含む)
 * 国際興業バスは対象外(既存の実データ投稿のため、このスクリプトでは触れません)
 *
 * 使い方:
 * 1. Code Snippets で「新規追加」→ このコード全体を貼り付けて保存・有効化
 * 2. 管理者アカウントでログインした状態で、ブラウザで以下のURLを1回だけ開く:
 *    https://busjob.net/wp-admin/?run_job_layout_update=1
 * 3. 「更新できた件数」と「見つからなかったタイトル」の一覧が出れば完了です
 * 4. 完了したら、このスニペットは Code Snippets の一覧で無効化(Deactivate)しておいてください
 *
 * 差し替え後も投稿ステータス(下書き/公開)は変更しません。
 * 【要確認】のプレースホルダーが多く残っているので、内容を仕上げてから公開してください。
 */

add_action( 'admin_init', function () {

	if ( empty( $_GET['run_job_layout_update'] ) ) {
		return;
	}
	...
	if ( get_option( 'bus_job_layout_update_done' ) ) {
		wp_die( 'レイアウトの一括差し替えは既に実行済みです(bus_job_layout_update_done オプション)。もう一度やり直したい場合は、管理画面から wp_options の bus_job_layout_update_done を削除してください。' );
	}
...
BJART48
	...
	update_option( 'bus_job_layout_update_done', current_time( 'mysql' ) );

	echo '<h1>' . count( $updated ) . ' 件のレイアウトを更新しました</h1><ul>';
	...
	if ( $not_found ) {
		...
		echo '</ul><p>投稿タイトルが変更されている場合、この一覧に出ます。該当があれば教えてください。</p>';
	}

	echo '<p>差し替え後も【要確認】のプレースホルダーが多く残っています。内容を仕上げてから公開してください。</p>';
	exit;
} );
